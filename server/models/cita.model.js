...
class CitaModel {
	...
	/**
	 * @method fetchAgendaDisponible
	 * @description Método para obtener las horas disponibles de un especialista en una fecha específica.
	 * @static
	 * @async
	 * @memberof CitaModel
	...
	static async fetchAgendaDisponible(searchValues, horas, dbConn) {
		...
		try {
			...
			const actualPage = page;
			const totalPages = Math.ceil(total / limit);
			const paginatedHorasLibres = horasLibres.slice(offset, offset + limit);
	...
	/**
	 * @method fetchByDataPaciente
	 * @description Método para obtener una cita por su fecha, hora y ID del paciente.
	 * @static
	 * @async
	 * @memberof CitaModel
	 * @param data - Los datos de búsqueda. Contiene la fecha, la hora y el ID del paciente.
	 * @param {Object} dbConn - La conexión a la base de datos.
	 * @returns {Promise<Object>} La cita que coincide con los datos de búsqueda.
	 * @throws {Error} Si ocurre un error durante la operación, se lanzará un error.
	 */
	static async fetchByDataPaciente(data, dbConn) {
		const query =
			'SELECT * ' +
			'FROM ' +
			'		cita ' +
