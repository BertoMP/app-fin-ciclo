...
export class SolicitarCitaComponent implements OnInit {
  ...
  buscarEspecialidades() {
    this.especialidadService.getEspecialidad().subscribe({
      next: (especialidad: EspecialidadModel[]) => {
        this.especialidades = especialidad.map(
  ...
  buscarEspecialistas() {
    ...
    if (this.especialidad_id) {
      this.medicalEspecialistService
        .getSpecialist(this.especialidad_id)
        .subscribe({
          next: (especialistas: EspecialistaCitaModel[]) => {
            if (especialistas.length === 0) {
              ...
              this.especialistas = especialistas.map(
                (especialista: EspecialistaCitaModel) => {
                  return {
  ...
  buscarCitas() {
    if (this.especialista_id && this.fecha) {
      this.citasService
        .getCitaDisponible(this.especialista_id, this.fecha)
        .subscribe({
